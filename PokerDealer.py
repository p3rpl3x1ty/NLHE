...
class PokerDealer:
    ...
    def player_actions(self):
        ...
        while not all_acted:
            ...
            action, amount = self.players_list[next_to_act].act(current_max_bet) #Actual Call to the Player.Act() function.
            if amount > current_max_bet:
                current_max_bet = amount

            # if action == bet we need to set everyone elses acted to False
            if action == 'bet':
                for player in range(self.players):
                    if player != next_to_act:
                        self.players_list[player].has_acted = False
            self.pot += amount
            
            next_to_act = (next_to_act + 1) % self.players
            all_acted = all([player.acted for player in self.players_list.itervalues()])
        if sum([player.has_folded() for player in self.players_list.itervalues()]) == (self.players - 1):
            return
        self.next_street()
